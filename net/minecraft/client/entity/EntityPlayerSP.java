...
public class EntityPlayerSP extends AbstractClientPlayer
{
	...
	public void moveEntity(double x, double y, double z) {
    	...
    	// So targetstrafe works
    	if (Vergo.config.modTargetStrafe.isEnabled())
    		Vergo.config.modTargetStrafe.onEvent(event);
    ...
    public void onUpdateWalkingPlayer()
    {
        ...
        if (this.isCurrentViewEntity())
        {
            ...
            {
                this.sendQueue.addToSendQueue(new C03PacketPlayer.C06PacketPlayerPosLook(motionX, -999.0D, motionZ, e.getYaw(), e.getPitch(), e.isOnGround()));
                flag2 = false;
            }
    ...
    {
    	EventChatMessage chatPre = new EventChatMessage();
    	chatPre.setType(EventType.PRE);
    	chatPre.setMessage(message);
    	chatPre.fire();
    	message = chatPre.getMessage();
    	if (chatPre.isCanceled())
    		return;
//        this.sendQueue.addToSendQueue(new C01PacketChatMessage(message));
    	mc.getNetHandler().getNetworkManager().sendPacketNoEvent(new C01PacketChatMessage(message));
    	EventChatMessage chatPost = new EventChatMessage();
    	chatPost.setType(EventType.POST);
    	chatPost.setMessage(message);
    	chatPost.fire();
    	message = chatPost.getMessage();
    	if (chatPost.isCanceled())
    		return;
    }
    ...
    public void onLivingUpdate()
    {
        ...
        event.setType(EventType.PRE);
        event.fire();
        slowDown = !event.isCanceled();
        if (this.isUsingItem() && !this.isRiding() && slowDown)
        {
