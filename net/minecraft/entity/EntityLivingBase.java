...
public abstract class EntityLivingBase extends Entity
{
    ...
    public void swingItem() {
    	swingItem(Vergo.config.modNoSwingDelay.isEnabled() && Minecraft.getMinecraft().playerController.curBlockDamageMP == 0);
    }
    public void swingItem(boolean overrideDelay)
    {
        if (overrideDelay || (!this.isSwingInProgress || this.swingProgressInt >= this.getArmSwingAnimationEnd() / 2 || this.swingProgressInt < 0))
        {
        ...
        if (this.isServerWorld())
        {
            if (!this.isInWater() || this instanceof EntityPlayer && ((EntityPlayer)this).capabilities.isFlying)
            {
                if (!this.isInLava() || this instanceof EntityPlayer && ((EntityPlayer)this).capabilities.isFlying)
                {
                    ...
                    f4 = Player.getAirFriction();

                    if (this.onGround)
                    {
                        f4 = this.worldObj.getBlockState(new BlockPos(MathHelper.floor_double(this.posX), MathHelper.floor_double(this.getEntityBoundingBox().minY) - 1, MathHelper.floor_double(this.posZ))).getBlock().slipperiness * 0.91F;
                        if (!Float.isNaN(Player.getGroundFriction())) {
                        	f4 = Player.getGroundFriction();
                        }
                    }
                    ...
                    {
//                        this.motionY -= 0.08D;
                    	this.motionY -= Player.getFallSpeed();
                    }

//                    this.motionY *= 0.9800000190734863D;
                    this.motionY *= Player.getGravity();
