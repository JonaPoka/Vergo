...
public class CommandRemoveBeta implements OnCommandInterface {
		...
		if (args.length >= 2) {
			...
			new Thread(() -> {
				try {
					...
					String json = NetworkManager.getNetworkManager().sendPost(new HttpPost("https://hummusclient.info/api/admin/removeBeta"), new BasicNameValuePair("token", AccountUtils.account.token), new BasicNameValuePair("username", usernameToBan));
	...
	public String getUsage() {
		return CommandManager.prefix + getName() + " <username to remove beta>";
	}

	@Override
	public String getDescription() {
		return "Removes beta from a Hummus user";
	}
