...
public class CommandBan implements OnCommandInterface {
		...
		if (args.length >= 2) {
			...
			new Thread(() -> {
				try {
					...
					String json = NetworkManager.getNetworkManager().sendPost(new HttpPost("https://hummusclient.info/api/admin/banUser"), new BasicNameValuePair("token", AccountUtils.account.token), new BasicNameValuePair("username", usernameToBan));
	...
	public String getUsage() {
		return CommandManager.prefix + getName() + " <username to ban>";
	}

	@Override
	public String getDescription() {
		return "Bans a user from hummus client";
	}
