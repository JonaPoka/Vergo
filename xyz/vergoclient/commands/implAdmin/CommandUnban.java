...
public class CommandUnban implements OnCommandInterface {
		...
		if (args.length >= 2) {
			...
			new Thread(() -> {
				try {
					...
					String json = NetworkManager.getNetworkManager().sendPost(new HttpPost("https://hummusclient.info/api/admin/unbanUser"), new BasicNameValuePair("token", AccountUtils.account.token), new BasicNameValuePair("username", usernameToBan));
	...
	public String getUsage() {
		return CommandManager.prefix + getName() + " <username to unban>";
	}

	@Override
	public String getDescription() {
		return "Unbans a user from hummus client";
	}
