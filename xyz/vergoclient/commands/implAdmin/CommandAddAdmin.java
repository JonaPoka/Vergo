...
public class CommandAddAdmin implements OnCommandInterface {
		...
		if (args.length >= 2) {
			...
			new Thread(() -> {
				try {
					...
					String json = NetworkManager.getNetworkManager().sendPost(new HttpPost("https://hummusclient.info/api/admin/addAdmin"), new BasicNameValuePair("token", AccountUtils.account.token), new BasicNameValuePair("username", usernameToBan));
	...
	public String getUsage() {
		return CommandManager.prefix + getName() + " <username to add admin>";
	}

	@Override
	public String getDescription() {
		return "Adds admin to a Hummus user";
	}
