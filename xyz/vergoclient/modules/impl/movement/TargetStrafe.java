...
public class TargetStrafe extends Module implements OnEventInterface {
	...
	public void onEvent(Event e) {
		...
		if ((e instanceof EventMove || e instanceof EventRender3D) && Vergo.config.modKillAura.isEnabled() && KillAura.target != null) {
			...
			for (short i = 0; i <= getAmountOfPoints() * 2; i++) {
				...
				if (lastLine == null) {
					lastLine = new Vec3(x2, KillAura.target.posY, z2);
					continue;
				}
	...
	public void circleEvent(Event e) {
		if (e instanceof EventMove && e.isPre()) {
			...
			if (KillAura.target == null || Vergo.config.modKillAura.isDisabled()) {
				return;
			} else {
				...
				float f = (float) ((RotationUtils.getRotations(KillAura.target)[0]
		...
		if (e instanceof EventRender3D && e.isPre()) {
			...
			if (KillAura.target == null || Vergo.config.modKillAura.isDisabled()) {
				...
				for (short i = 0; i <= 360 * 2; i++) {
					...
					float f = (float) ((RotationUtils.getRotations(KillAura.target)[0] + (direction ? -i : i)) * (Math.PI / 180));
					double x2 = KillAura.target.posX, z2 = KillAura.target.posZ;
					x2 -= (double) (MathHelper.sin(f) * distanceSetting.getValueAsDouble()) * -1;
					z2 += (double) (MathHelper.cos(f) * distanceSetting.getValueAsDouble()) * -1;
